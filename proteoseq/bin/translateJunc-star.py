...
def main():
	...
	junction_reads = junctionReads(bamfile,HSExonfile)
...
def junctionReads(sam_fn,hsexonfile):  # bam file input
# store reads id that mapped to human_epcific_exon to a dict
	exonid = {}
	d = subprocess.Popen('samtools view -b -q 255 ' + sam_fn + ' | bedtools intersect -a ' + hsexonfile + ' -b stdin -wa -wb -split -sorted', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
